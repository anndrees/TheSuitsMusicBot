...
module.exports = {
  ...
  cooldown: 1.5,
  edesc: `Escribe este comando para poner algo de musica.\nModo de uso: ${PREFIX}play <TITULO | LINK>`,
...
async execute(message, args, client) {
    ...
    //If no args return
    if (!args.length)
      return attentionembed(message, `Modo de uso: ${message.client.prefix}play <YouTube URL | Nombre del Video | Soundcloud URL>`);
    //react with approve emoji
    message.react(approveemoji).catch(console.error);
    //get permissions and send error if bot doesnt have enough
    const permissions = channel.permissionsFor(message.client.user);
    if (!permissions.has("CONNECT"))
      return attentionembed(message, "Necesito permisos para entrar a tu canal de voz!");
    if (!permissions.has("SPEAK"))
      return attentionembed(message, "Necesito permisos para hablar en tu canal de voz!");
      ...
      if (serverQueue) {
        ...
        if (urlValid) { //send searching link
          message.channel.send(new MessageEmbed().setColor("#F0EAD6")
            .setDescription(`**:notes: Buscando 🔍 [\`LINK\`](${args.join(" ")})**`))
        //if not
        }
        else { //send searching TITLE
          message.channel.send(new MessageEmbed().setColor("#F0EAD6")
            .setDescription(`**:notes: Buscando 🔍 \`${args.join(" ")}\`**`))
        }
      } else {
        ...
        //send join message
        message.channel.send(new MessageEmbed().setColor("#F0EAD6")
          .setDescription(`**👍 Unido a \`${channel.name}\` 📄 desde \`#${message.channel.name}\`**`)
    ...
    if (serverQueue) {
      ...
      if (estimatedtime > 60) {
        ...
        estimatedtime = Math.round(estimatedtime / 60 * 100) / 100;
        estimatedtime = estimatedtime + " Horas"
      }
      else {
      ...
      const newsong = new MessageEmbed()
        ...
        .setThumbnail(thumb)
        .setURL(song.url)
        .setDescription(`\`\`\`Ha sido añadida a la lista.\`\`\``)
        .addField("Tiempo estimado hasta reproduccion:", `\`${estimatedtime}\``, true)
        .addField("Posicion en lista", `**\`${serverQueue.songs.length - 1}\`**`, true)
    ...
    try {
      ...
      return attentionembed(message, `No puedo entrar al canal: ${error}`);
    }
  }
};
