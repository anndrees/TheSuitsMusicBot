...
module.exports = {
  name: "filter",
  description: "Aplica efectos de audio",
  aliases: ["f"],
  cooldown: 3,
  edesc: `Escribe este comando para cambiar el efecto de audio actual \nModo de uso: ${PREFIX}filter <TipoDeFiltro>`,
  ...
  async execute(message, args, client) {
    ...
    if (!message.member.voice.channel)
      return attentionembed(message, "Por favor entra a una canal de voz primero")
    ...
    switch (args[0]) {
      ...
      default:
        ...
        .setColor("#F0EAD6")
        .setTitle("Filtro no valido, usa uno de estos:")
        .setDescription(`
        \`bassboost\`
        \`8D\`
        \`vaporwave\`
        ...
        \`chorus\`
        \`karaoke\`
        \`sofa (Hace que el audio sea adecuado para auriculares/cascos)\`
        \`desilencer (Elimina el silencio de la cancion automaticamente)\`
        \`clear\`   ---  Quita todos los filtros`)
        .setFooter(`Ejemplo: ${PREFIX}filter bassboost`)
        )
        break;
    }
    //set choice to zero
    ...
    try {
      ...
      message.channel.send(new MessageEmbed()
      .setColor("#F0EAD6")
      .setAuthor("Aplicando: " + args[0], "https://i.redd.it/y3wduhwn4gd61.jpg")).then(msg =>{
