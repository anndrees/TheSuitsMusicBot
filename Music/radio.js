...
module.exports = {
  ...
  cooldown: 3,
  edesc: `Escribe este comando para reproducir una radio en directo!\nModo de uso: ${PREFIX}radio <1-34>`,
  async execute(message, args, client) {
    //define the No args Embed, lmao
    let resultsEmbed = new Discord.MessageEmbed()
      .setTitle(`**Radio Disponibles**`)//
      .addFields(
        {
...
**34: ** [\`${Radiostations[34 - 1].split(" ")[0]}\`](${Radiostations[34 - 1].split(" ")[1]})`, inline: true
    ...
    if (!permissions.has("CONNECT"))
      return attentionembed(message, "Necesito permisos para entrar a tu canal!");
    if (!permissions.has("SPEAK"))
      return attentionembed(message, "Necesito permisos para hablar en tu canal!");
    ...
    if (isNaN(args[0])) {
      ...
      return message.reply(
        new MessageEmbed()
          .setColor("#F0EAD6")
          .setTitle(`Radio no valida por favor usa un numero entre \`1\` y \`${Radiostations.length}\``)
      );
    }
